...
$('#ft-import-url').on('submit', function(e) {
  ...
  // CLICK en “Ejecutar Benchmark”
  $(document).on('click', '#ft-benchmark-btn', function(e){
    e.preventDefault();
    ...
    // Leer valores
    var season    = $('#benchmark-season').val();
    var modelType = $('#model-type').val();
    var league    = $('#benchmark-league').val() || 'all';
    ...
    $.ajax({
      ...
      error: function(xhr, status, err){
        ...
        var msg = (xhr.responseJSON && xhr.responseJSON.data)
                  ...
                  : err;
        $('#ft-benchmark-results')
          .html('<div class="notice notice-error"><p>❌ Error Benchmark: '+ msg +'</p></div>')
          .show();
      }
    });
